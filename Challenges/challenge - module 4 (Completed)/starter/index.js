...
// Turning the array above into numbers. 
...
// Finding out the net total amount of profit/losses.
let total = 0;
...
for (let i = 0; i < finances.length; i++) {
  for (let j = 0; j< finances.length; j++) {
    if (finances[i][j] > largest) {
      largest = finances[i][j];
    }
  }
}

console.log('Greatest Increase in Profits/Losses: ' + largest);

// Finding out the smallest value

var smallest = finances[0][1];

for (let i = 0; i < finances.length; i++) {
  for (let j = 0; j< finances.length; j++) {
    if (finances[i][j] < smallest) {
      smallest = finances[i][j];
    }
  }
}
